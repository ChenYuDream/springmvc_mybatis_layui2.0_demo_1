layui.define(['layer', 'element', 'fortree'], function (exports) {
    ...
    function bind(height) {
        var height = height || 60 + 41; //头部高度 顶部切换卡标题高度 底部高度
        /**
         * iframe自适应
         */
        ...
        element.on('nav(' + navfilter + ')', function (elem) {
            ...
            if (src != undefined && src != null && id != undefined && id != null) {
                if (iframe.length) { //存在 iframe
                    ...
                    document.getElementById(tabindex).contentWindow.location.reload(true);
                } else { //不存在 iframe
                    //显示加载层
                    ...
                    //拼接iframe
                    var iframe = '<iframe id="' + tabindex + '" onload="layui.layer.close(' + tmpIndex + ')"';
